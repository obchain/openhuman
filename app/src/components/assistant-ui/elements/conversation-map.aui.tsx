 ...
 * Source: https://r.assistant-ui.com/styles/base-nova/conversation-map.json
 */
'use client';

import { cn } from '@/components/assistant-ui/lib/utils';
import { type ThreadMessage, useAuiState, useThreadViewport } from '@assistant-ui/react';
...
const describe = ({ head, members }: Turn): ConversationMapEntry => {
  ...
  // What the turn asked names it; the complete assistant response is the
  // useful hover preview. A response can span multiple assistant messages, so
  // collect every textual assistant member rather than stopping at the first.
  // A turn still being answered falls back to the remainder of its own text.
  const response = members
    .filter(member => member !== head && member.role === 'assistant')
    .flatMap(linesOf)
    .join(' ')
    .trim();
  const preview = (response || [first.slice(title.length), ...lines.slice(1)].join(' '))
    .trim()
